...
import {
  ...
  updateExchangePolicyStatus,
  getShowList,
  type ExchangePolicy as ExchangePolicyDto,
  type SaveExchangePolicyRequest,
  type ShowDto,
} from '../../../api/admin'
...
const ExchangePolicy = () => {
  const [data, setData] = useState<ExchangePolicyDto[]>([])
  const [shows, setShows] = useState<ShowDto[]>([])
  const [loading, setLoading] = useState(false)
  const [modalVisible, setModalVisible] = useState(false)
  const [editingItem, setEditingItem] = useState<ExchangePolicyDto | null>(null)
  ...
  const loadData = async (page = 1, pageSize = 10) => {
    setLoading(true)
    try {
      const res = await getExchangePolicyList({
        ShowId: showFilter,
        Status: statusFilter,
        Page: page,
        PageSize: pageSize,
  ...
  const openCreate = () => {
    setEditingItem(null)
    form.resetFields()
    form.setFieldsValue({ exchangeFee: 0, allowCrossSession: false, priority: 0 })
    setModalVisible(true)
  }

  const openEdit = (item: ExchangePolicyDto) => {
    setEditingItem(item)
    form.resetFields()
    form.setFieldsValue({
      showId: item.showId != null ? Number(item.showId) : undefined,
      policyName: item.policyName,
      exchangeDeadlineHour: Number(item.exchangeDeadlineHour),
      exchangeFee: Number(item.exchangeFee),
      allowCrossSession: Number(item.allowCrossSession) === 1,
  ...
  const handleSave = async () => {
    try {
      ...
      const payload: SaveExchangePolicyRequest = {
        showId: values.showId ?? null,
        policyName: values.policyName.trim(),
        exchangeDeadlineHour: values.exchangeDeadlineHour,
        exchangeFee: values.exchangeFee ?? 0,
        allowCrossSession: values.allowCrossSession ? 1 : 0,
        priority: values.priority ?? 0,
        remark: values.remark?.trim() || null,
      }
      setSaving(true)
      const res = editingItem
        ? await updateExchangePolicy(Number(editingItem.policyId), payload)
        : await createExchangePolicy(payload)
      if (res.error) {
        message.error('保存失败')
        return
      }
  ...
  const handleToggleStatus = async (item: ExchangePolicyDto, status: number) => {
    try {
      const res = await updateExchangePolicyStatus(Number(item.policyId), { status })
      if (res.error) {
        message.error('操作失败')
        return
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'show',
      width: 220,
      render: (_: unknown, record: ExchangePolicyDto) => {
  ...
  return (
    <div>
      ...
      <Modal
        title={editingItem ? '编辑改签策略' : '新建改签策略'}
        open={modalVisible}
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 8 }}>
          <Form.Item name="policyName" label="策略名称" rules={[{ required: true, message: '请输入策略名称' }]}>
            <Input placeholder="如：开场前3小时可改签" maxLength={50} />
          </Form.Item>
          ...
          <Form.Item
            name="exchangeDeadlineHour"
            label="改签截止（距演出开始小时数）"
            rules={[{ required: true, message: '请输入截止小时数' }]}
          >
            <InputNumber min={0} max={8760} style={{ width: '100%' }} addonAfter="小时" />
          </Form.Item>
          <Form.Item name="exchangeFee" label="改签费（元）">
            <InputNumber min={0} step={0.5} style={{ width: '100%' }} />
          </Form.Item>
          <Form.Item name="allowCrossSession" label="允许跨场次改签" valuePropName="checked">
            <Switch />
          </Form.Item>
